...
header_replacement = """<div class="page-header" style="display: flex; justify-content: space-between; align-items: center; flex-wrap: wrap; gap: 10px;">
        <h1>Exams List</h1>
        <div>
            {% if request.user|has_permission:'delete-exams' %}
            <button type="submit" form="bulkDeleteForm" class="btn btn-danger mr-2" id="bulkDeleteBtn" disabled onclick="return confirm('Are you sure you want to delete selected exams?')">
                <i class="fas fa-trash"></i> Delete Selected
            </button>
            {% endif %}
            {% if request.user|has_permission:'create-exams' %}
            <a href="{% url 'exams_create' %}" class="btn btn-primary">
...
table_replacement = """<form id="bulkDeleteForm" method="POST" action="{% url 'exams_bulk_delete' %}">
            {% csrf_token %}
...
content = content.replace('<table id="examsTable" class="display" style="width:100%">\n            <thead>\n                <tr>', table_replacement)

tbody_replacement = """<tbody>
                {% for exam in exams %}
                <tr>
                    <td class="text-center">
                        <input type="checkbox" name="ids" class="bulk-checkbox" value="{{ exam.id }}">
                    </td>"""
...
js_replacement = """    $(document).ready(function () {
        ...
        $('#examsTable tbody').on('change', 'input[type="checkbox"].bulk-checkbox', function() {
