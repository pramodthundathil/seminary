...
with open('templates/admin/languages/languages_list.html', 'r') as f:
    content = f.read()
...
header_replacement = """<div class="page-header" style="display: flex; justify-content: space-between; align-items: center; flex-wrap: wrap; gap: 10px;">
        <h1>Language List</h1>
        <div>
            <button type="submit" form="bulkDeleteForm" class="btn btn-danger mr-2" id="bulkDeleteBtn" disabled onclick="return confirm('Are you sure you want to delete selected languages?')">
...
table_replacement = """<form id="bulkDeleteForm" method="POST" action="{% url 'language_bulk_delete' %}">
            {% csrf_token %}
...
tbody_replacement = """<tbody>
                ...
                <tr>
                    <td class="text-center">
                        <input type="checkbox" name="ids" class="bulk-checkbox" value="{{ language.id }}">
                    </td>"""
...
with open('templates/admin/languages/languages_list.html', 'w') as f:
    f.write(content)
