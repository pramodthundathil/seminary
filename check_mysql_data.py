...
def check_table_counts():
    ...
    try:
        mysql_cursor.execute("""
            SELECT id, first_name, last_name, user_id, course_applied_id, 
                   language_id, citizenship_id, country_id
            FROM students 
            LIMIT 5
        """)
        ...
        if rows:
            print(f"{'ID':<6} {'Name':<25} {'User':<6} {'Course':<7} {'Lang':<5} {'Citizen':<7} {'Country':<7}")
            print("-"*70)
            
            for row in rows:
                id_, first, last, user_id, course_id, lang_id, citizen_id, country_id = row
                name = f"{first} {last or ''}"[:25]
                print(f"{id_:<6} {name:<25} {user_id or 'NULL':<6} {course_id or 'NULL':<7} {lang_id or 'NULL':<5} {citizen_id or 'NULL':<7} {country_id or 'NULL':<7}")
            ...
            for row in rows:
                ...
                # Check Course
                if course_id:
                    sqlite_cursor.execute(f"SELECT COUNT(*) FROM courses WHERE id = {course_id}")
                    exists = sqlite_cursor.fetchone()[0] > 0
                    print(f"  Course {course_id}: {'✓ EXISTS' if exists else '✗ MISSING'}")
